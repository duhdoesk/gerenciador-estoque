...
def saida_produtos(lista_id, lista_quantidade, destino, data = ''):
	...
	while i < len(lista_id):
		...
		fprod.carga(lista_id[i], lista_quantidade[i], destino, data)
		# inserindo informações no registro
		tempo = time()
...
def format_list(estoque, id_item, item, c01='', c02='',c03=''):
	estoque = str(estoque)
	id_item = str(id_item)
	item = str(item)
	c01 = str(c01)
...
def imprime_lista(tabela, opcao='1'):
	...
	if tabela == 'materiais':
		...
		print(format_list('estoque', 'id', 'material') + '\n' + ('-' * 80))
		for item in itens:
			print(format_list(item[-1], item[0], item[1]))
		print('\n' + ('#' * 80) + '\n')	
	else:
		print('\n' + ('#' * 80) + '\n', '\n' + 'Opção: Imprimir lista de produtos em estoque' + '\n')
		print(format_list('estoque', 'id', 'produto', 'condição', 'aba', 'cor') + '\n' + ('-' * 80))
		for item in itens:
			print(format_list(item[-1], item[0], item[1], item[2], item[3], item[4]))
		print('\n' + ('#' * 80) + '\n')
